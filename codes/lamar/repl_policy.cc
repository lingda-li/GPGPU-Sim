...
void tag_array::decide_prefetch(mem_fetch *mf, new_addr_type addr)
{
    ...
    for(unsigned i = 0; i < 6; i++) {
        if(m_cur_hash_bit_array[hash_function(i, addr)] == false) {
            found = false;
            break;
        }
    }

    if((!found && m_default_prediction) ||
       (found && !m_default_prediction)) { // bypass is used
        m_bypasses++;
    } else {
...
{
    assert(m_config.m_replacement_policy == SECTOR);
    ...
    for(unsigned i = 0; i < m_config.get_line_sz()/CACHE_CHUNK_SIZE; i++) {
        for(unsigned j = 0; j < CACHE_CHUNK_SIZE; j++)
            if(m_lines[victim_idx].m_reuse.test(i*CACHE_CHUNK_SIZE+j)) {
                chunk_mask.set(i);
                break;
            }
    }
    m_total_victim++;
    if((chunk_mask.count() >= 2 && m_default_prediction) ||
       (chunk_mask.count() < 2 && !m_default_prediction)) {
        for(unsigned i = 0; i < 6; i++) {
            m_hash_bit_array0[hash_function(i, victim_addr)] = true;
            m_hash_bit_array1[hash_function(i, victim_addr)] = true;
        }
        m_hash_insertion++;
    }

    // reset hash table
    if(m_total_victim % 512 == 0) {
        for(unsigned i = 0; i < 2048; i++)
            m_cur_hash_bit_array[i] = false;
        if(m_cur_hash_bit_array == m_hash_bit_array0)
            m_cur_hash_bit_array = m_hash_bit_array1;
        else
            m_cur_hash_bit_array = m_hash_bit_array0;
        if((double)m_hash_insertion / (double)m_total_victim >= 0.7) {
            m_total_victim = 0;
            m_hash_insertion = 0;
            m_default_prediction = !m_default_prediction;
            for(unsigned i = 0; i < 2048; i++) {
                m_hash_bit_array0[i] = false;
                m_hash_bit_array1[i] = false;
            }
        }
    }
...
void tag_array::get_sub_stats(struct cache_sub_stats &css) const{
    ...
    t_css.repl_stats[INTRA_HIT] = m_total_victim;
    t_css.repl_stats[INTER_HIT] = m_hash_insertion;
...
void tag_array::init_repl()
{
    ...
    //m_default_prediction = true;
    m_default_prediction = false;
    m_hash_bit_array0 = new bool[2048];
    m_hash_bit_array1 = new bool[2048];
    for(unsigned i = 0; i < 2048; i++) {
        m_hash_bit_array0[i] = false;
        m_hash_bit_array1[i] = false;
    }
...
new_addr_type BypassMonitor::GetBDaddr(unsigned &tid, new_addr_type PC, new_addr_type addr, new_addr_type vaddr, int sign, unsigned accessType)
{
    ...
    if(sign > 0)
        ...
        bd_addr += ((sign + (int)(linesize/CACHE_CHUNK_SIZE) + 1) << m_bd_addr_len);
...
unsigned get_key_bit(unsigned bit, unsigned key)
{
    return (key >> bit) & 1;
}

unsigned tag_array::hash_function(int idx, unsigned key)
{
    key = (key >> m_config.m_line_sz_log2);
    key &= ((1 << 22) - 1);
    unsigned res = 0;
    if(idx == 0) {
        res  = get_key_bit( 0, key) ^ get_key_bit(21, key); res = (res << 1);
        res |= get_key_bit( 1, key) ^ get_key_bit(20, key); res = (res << 1);
        res |= get_key_bit( 2, key) ^ get_key_bit(19, key); res = (res << 1);
        res |= get_key_bit( 3, key) ^ get_key_bit(18, key); res = (res << 1);
        res |= get_key_bit( 4, key) ^ get_key_bit(17, key); res = (res << 1);
        res |= get_key_bit( 5, key) ^ get_key_bit(16, key); res = (res << 1);
        res |= get_key_bit( 6, key) ^ get_key_bit(15, key); res = (res << 1);
        res |= get_key_bit( 7, key) ^ get_key_bit(14, key); res = (res << 1);
        res |= get_key_bit( 8, key) ^ get_key_bit(13, key); res = (res << 1);
        res |= get_key_bit( 9, key) ^ get_key_bit(12, key); res = (res << 1);
        res |= get_key_bit(10, key) ^ get_key_bit(11, key);
    } else if(idx == 1) {
        res  = get_key_bit( 0, key) ^ get_key_bit(11, key); res = (res << 1);
        res |= get_key_bit( 1, key) ^ get_key_bit(12, key); res = (res << 1);
        res |= get_key_bit( 2, key) ^ get_key_bit(13, key); res = (res << 1);
        res |= get_key_bit( 3, key) ^ get_key_bit(14, key); res = (res << 1);
        res |= get_key_bit( 4, key) ^ get_key_bit(15, key); res = (res << 1);
        res |= get_key_bit( 5, key) ^ get_key_bit(16, key); res = (res << 1);
        res |= get_key_bit( 6, key) ^ get_key_bit(17, key); res = (res << 1);
        res |= get_key_bit( 7, key) ^ get_key_bit(18, key); res = (res << 1);
        res |= get_key_bit( 8, key) ^ get_key_bit(19, key); res = (res << 1);
        res |= get_key_bit( 9, key) ^ get_key_bit(20, key); res = (res << 1);
        res |= get_key_bit(10, key) ^ get_key_bit(21, key);
    } else if(idx == 2) {
        res  = get_key_bit(10, key) ^ get_key_bit(20, key); res = (res << 1);
        res |= get_key_bit( 9, key) ^ get_key_bit(19, key); res = (res << 1);
        res |= get_key_bit( 8, key) ^ get_key_bit(18, key); res = (res << 1);
        res |= get_key_bit( 7, key) ^ get_key_bit(17, key); res = (res << 1);
        res |= get_key_bit( 6, key) ^ get_key_bit(16, key); res = (res << 1);
        res |= get_key_bit( 5, key) ^ get_key_bit(15, key); res = (res << 1);
        res |= get_key_bit( 4, key) ^ get_key_bit(14, key); res = (res << 1);
        res |= get_key_bit( 3, key) ^ get_key_bit(13, key); res = (res << 1);
        res |= get_key_bit( 2, key) ^ get_key_bit(12, key); res = (res << 1);
        res |= get_key_bit( 1, key) ^ get_key_bit(11, key); res = (res << 1);
        res |= get_key_bit( 0, key) ^ get_key_bit(21, key);
    } else if(idx == 3) {
        res  = get_key_bit(10, key) ^ get_key_bit(13, key); res = (res << 1);
        res |= get_key_bit( 9, key) ^ get_key_bit(14, key); res = (res << 1);
        res |= get_key_bit( 8, key) ^ get_key_bit(15, key); res = (res << 1);
        res |= get_key_bit( 7, key) ^ get_key_bit(16, key); res = (res << 1);
        res |= get_key_bit( 6, key) ^ get_key_bit(17, key); res = (res << 1);
        res |= get_key_bit( 5, key) ^ get_key_bit(18, key); res = (res << 1);
        res |= get_key_bit( 4, key) ^ get_key_bit(19, key); res = (res << 1);
        res |= get_key_bit( 3, key) ^ get_key_bit(20, key); res = (res << 1);
        res |= get_key_bit( 2, key) ^ get_key_bit(21, key); res = (res << 1);
        res |= get_key_bit( 1, key) ^ get_key_bit(11, key); res = (res << 1);
        res |= get_key_bit( 0, key) ^ get_key_bit(12, key);
    } else if(idx == 4) {
        res  = get_key_bit( 0, key) ^ get_key_bit(11, key); res = (res << 1);
        res |= get_key_bit( 4, key) ^ get_key_bit(12, key); res = (res << 1);
        res |= get_key_bit( 5, key) ^ get_key_bit(20, key); res = (res << 1);
        res |= get_key_bit( 6, key) ^ get_key_bit(21, key); res = (res << 1);
        res |= get_key_bit( 1, key) ^ get_key_bit(13, key); res = (res << 1);
        res |= get_key_bit( 2, key) ^ get_key_bit(14, key); res = (res << 1);
        res |= get_key_bit( 3, key) ^ get_key_bit(15, key); res = (res << 1);
        res |= get_key_bit( 9, key) ^ get_key_bit(16, key); res = (res << 1);
        res |= get_key_bit(10, key) ^ get_key_bit(17, key); res = (res << 1);
        res |= get_key_bit( 7, key) ^ get_key_bit(18, key); res = (res << 1);
        res |= get_key_bit( 8, key) ^ get_key_bit(19, key);
    } else if(idx == 5) {
        res  = get_key_bit( 6, key) ^ get_key_bit(11, key); res = (res << 1);
        res |= get_key_bit( 1, key) ^ get_key_bit(21, key); res = (res << 1);
        res |= get_key_bit( 2, key) ^ get_key_bit(20, key); res = (res << 1);
        res |= get_key_bit( 3, key) ^ get_key_bit(12, key); res = (res << 1);
        res |= get_key_bit( 0, key) ^ get_key_bit(13, key); res = (res << 1);
        res |= get_key_bit( 4, key) ^ get_key_bit(19, key); res = (res << 1);
        res |= get_key_bit( 5, key) ^ get_key_bit(18, key); res = (res << 1);
        res |= get_key_bit( 9, key) ^ get_key_bit(17, key); res = (res << 1);
        res |= get_key_bit(10, key) ^ get_key_bit(16, key); res = (res << 1);
        res |= get_key_bit( 8, key) ^ get_key_bit(14, key); res = (res << 1);
        res |= get_key_bit( 7, key) ^ get_key_bit(15, key);
    } else
        abort();
    assert(res < 2048);
    return res;
