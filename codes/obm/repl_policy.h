...
class BypassMonitor
{
  ...
  public:
    ...
    void Fill(int set, int way, unsigned tid, address_type pc, new_addr_type addr, new_addr_type vaddr, int sign, mem_access_type type);
    int  LookUp(unsigned tid, new_addr_type addr);
    void LookUpVictim(unsigned tid, new_addr_type addr);
    ...
    unsigned GetSetIndex( new_addr_type addr )
    {
        if(m_cache_config.m_set_index_function == FERMI_HASH_SET_FUNCTION) {
            ...
            return ((lower_xor ^ upper_xor) & indexMask);
