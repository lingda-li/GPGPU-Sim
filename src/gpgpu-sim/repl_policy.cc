...
//#define BM_NO_BYPASS
//#define BM_NO_PREFETCH

#define BM_LOOKUP_VICTIM
...
unsigned tag_array::get_victim(mem_fetch *mf, new_addr_type addr)
{
    ...
    {
        unsigned extra_size = mf->get_alloc_sector_mask(m_core_id<0).count() - (m_config.m_assoc - m_sets[set_index].m_size);
        lruWay = Get_LRU_Victim_SizeAware(set_index,(unsigned)-1,extra_size);
    }
    ...
    bitset<MAX_CACHE_CHUNK_NUM> i_sector_mask = mf->get_original_sector_mask(m_core_id<0);
    if(BM_victim_way >= 0)
        BM->Fill(BM_victim_set, BM_victim_way, mf, tid, PC, addr, &m_lines[victim_idx], mf->get_sign(), i_sector_mask, accessType, m_core_id<0);

    if(is_bypass && !mf->get_alloc_sector_mask(m_core_id<0).count())
...
void tag_array::get_sub_stats(struct cache_sub_stats &css) const{
    ...
    t_css.desired_prefetches = m_desired_prefetch;
...
unsigned tag_array::get_partial_victim(mem_fetch *mf, new_addr_type addr, unsigned idx)
{
    ...
    {
        bitset<MAX_CACHE_CHUNK_NUM> sector_mask = mf->get_alloc_sector_mask(m_core_id<0) & ~m_lines[idx].m_sector_mask;
        unsigned extra_size = sector_mask.count() - (m_config.m_assoc - m_sets[set_index].m_size);
        lruWay = Get_LRU_Victim_SizeAware(set_index,idx,extra_size);
    }
    else if(BM->m_base_repl == BASE_REPL_NRU)
    ...
    int BM_victim_way = BM->Get_Victim(tid, BM_victim_set);
    bitset<MAX_CACHE_CHUNK_NUM> i_sector_mask = mf->get_original_sector_mask(m_core_id<0);
    //bitset<MAX_CACHE_CHUNK_NUM> i_sector_mask = mf->get_original_sector_mask(m_core_id<0) & (~m_lines[idx].m_sector_mask);
    if(BM_victim_way >= 0)
        BM->Fill(BM_victim_set, BM_victim_way, mf, tid, PC, addr, &m_lines[victim_idx], mf->get_sign(), i_sector_mask, accessType, m_core_id<0);
...
unsigned tag_array::get_real_victim(mem_fetch *mf, new_addr_type addr, unsigned idx)
{
    ...
    //int BM_victim_way = BM->Get_Victim(tid, BM_victim_set);
    //bitset<MAX_CACHE_CHUNK_NUM> i_sector_mask = mf->get_original_sector_mask(m_core_id<0);
    //if((m_lines[idx].m_status == VALID || m_lines[idx].m_status == MODIFIED || m_lines[idx].m_status == RESERVED) &&
    //   idx != (unsigned)-1)
    //    i_sector_mask &= ~m_lines[idx].m_sector_mask;
    //if(BM_victim_way >= 0)
    //    BM->Fill(BM_victim_set, BM_victim_way, mf, tid, PC, addr, &m_lines[victim_idx], mf->get_sign(), i_sector_mask, accessType, m_core_id<0);
...
int tag_array::Get_Random_Victim( unsigned setIndex, unsigned used_idx )
{
    ...
    for(way=0; way<m_assoc; way++) 
    {
        ...
        if(m_lines[idx].m_status != RESERVED && m_lines[idx].m_status != INVALID && m_lines[idx].m_status != UNUSED && idx != used_idx)
            avail_num++;
    }
    assert(avail_num > 0);
...
int tag_array::Get_LRU_Victim( unsigned setIndex, unsigned used_idx )
{
    ...
    {
        unsigned idx = setIndex * m_assoc + way;
        if( replSet[way].LRUstackposition >= maxLRUstackposition && m_lines[idx].m_status != RESERVED && m_lines[idx].m_status != INVALID && m_lines[idx].m_status != UNUSED && idx != used_idx )
        {
            lruWay = way;
...
int tag_array::Get_LRU_Victim_SizeAware( unsigned setIndex, unsigned used_idx, unsigned size )
{
    // Get pointer to replacement state of current set
    LINE_REPLACEMENT_STATE *replSet = repl[ setIndex ];
    ...
    {
        unsigned idx = setIndex * m_assoc + way;
        if( replSet[way].LRUstackposition >= maxLRUstackposition && m_lines[idx].m_status != RESERVED && m_lines[idx].m_status != INVALID && m_lines[idx].m_status != UNUSED && idx != used_idx && size == m_lines[idx].m_sector_mask.count())
        {
            lruWay = way;
            maxLRUstackposition = replSet[way].LRUstackposition;
        }
    ...
    if(lruWay >= 0)
        ...
        return Get_LRU_Victim(setIndex, used_idx);
}
...
int tag_array::Get_NRU_Victim( unsigned setIndex, unsigned used_idx )
{
    ...
    {
        unsigned idx = setIndex * m_assoc + way;
        if( replSet[way].NRUbit == false && m_lines[idx].m_status != RESERVED && m_lines[idx].m_status != INVALID && m_lines[idx].m_status != UNUSED && idx != used_idx )
            return way;
    }
...
int tag_array::Get_RRIP_Victim( unsigned setIndex, unsigned used_idx )
{
    ...
    {
        unsigned idx = setIndex * m_assoc + way;
        if(replSet[way].RRPV >= rrpv_max && m_lines[idx].m_status != RESERVED && m_lines[idx].m_status != INVALID && m_lines[idx].m_status != UNUSED && idx != used_idx)
        {
            replway = way;
            rrpv_max = replSet[way].RRPV;
        }
...
void BypassMonitor::Fill(int set, int way, mem_fetch *mf, unsigned tid, address_type pc, new_addr_type addr, cache_block_t *vic_blk, int sign, std::bitset<MAX_CACHE_CHUNK_NUM> sector_mask, mem_access_type type, bool l2)
{
    ...
    entries[set][way].i_sector_mask = sector_mask;
